      ...
      builder: (context, snapshot) {
        ...
        return ValueListenableBuilder<AppPhase>(
          ...
          builder: (context, phase, _) {
            return switch (phase) {
              AppPhase.needLicense => ActivationPage(controller: widget.controller),
              ...
              child: Padding(
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('Activation de licence', style: Theme.of(context).textTheme.headlineMedium),
                      const SizedBox(height: 12),
                      const Text('La licence est mémorisée localement et validée au démarrage.'),
                      const SizedBox(height: 24),
                      TextFormField(
                        controller: _licenseController,
                        decoration: const InputDecoration(labelText: 'Clé de licence'),
                        validator: (value) => (value == null || value.trim().isEmpty) ? 'La clé est obligatoire' : null,
                      ),
                      const SizedBox(height: 16),
                      TextFormField(
                        controller: _ownerController,
                        decoration: const InputDecoration(labelText: 'Nom du client'),
                        validator: (value) => (value == null || value.trim().isEmpty) ? 'Le client est obligatoire' : null,
                      ),
                      const SizedBox(height: 16),
                      if (_error != null) ...[
              ...
              child: Padding(
                ...
                child: Form(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: [
                      Text('Création du compte administrateur', style: Theme.of(context).textTheme.headlineMedium),
                      const SizedBox(height: 12),
                      const Text('Aucun utilisateur n’existe encore pour cette licence.'),
                      const SizedBox(height: 24),
                      TextFormField(
                      ...
                      TextFormField(
                        ...
                        validator: (value) => (value == null || value.trim().isEmpty) ? 'L’e-mail est obligatoire' : null,
                      ),
                      const SizedBox(height: 16),
                      TextFormField(
                        ...
                        obscureText: true,
                        validator: (value) => (value == null || value.length < 4) ? 'Le mot de passe doit contenir au moins 4 caractères' : null,
                      ),
                      const SizedBox(height: 16),
                      if (_error != null) ...[
                        Text(_error!, style: TextStyle(color: Theme.of(context).colorScheme.error)),
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      const Text('La licence est déjà validée. Connectez-vous pour accéder à l’application.'),
                      const SizedBox(height: 24),
                      TextFormField(
                        controller: _emailController,
                        decoration: const InputDecoration(labelText: 'E-mail'),
    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          ...
          NavigationDestination(icon: Icon(Icons.shopping_cart_outlined), selectedIcon: Icon(Icons.shopping_cart), label: 'Achats'),
          NavigationDestination(icon: Icon(Icons.inventory_2_outlined), selectedIcon: Icon(Icons.inventory_2), label: 'Stock'),
          NavigationDestination(icon: Icon(Icons.groups_outlined), selectedIcon: Icon(Icons.groups), label: 'Partenaires'),
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: AnimatedContainer(
                          ...
                          height: maxValue <= 0 ? 6 : (140 * (value / maxValue)).clamp(6, 140),
...
class _SalesPageState extends State<SalesPage> {
          ...
          children: [
            _FormCard(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  TextField(controller: _customerName, decoration: const InputDecoration(labelText: 'Client')),
                  const SizedBox(height: 12),
                  TextField(controller: _customerPhone, decoration: const InputDecoration(labelText: 'Téléphone client')),
                  const SizedBox(height: 12),
                    ...
                    items: products
                        .map(
                          (product) => DropdownMenuItem(
                            value: product.id,
                            child: Text('${product.name} - stock ${product.stockQuantity.toStringAsFixed(0)}'),
                          ),
                        )
                        .toList(growable: false),
                  ...
                  Row(
                    children: [
                      Expanded(child: TextField(controller: _quantityController, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Quantité'))),
                      const SizedBox(width: 12),
                      Expanded(child: TextField(controller: _priceController, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Prix unitaire'))),
                    ],
                  ),
                  const SizedBox(height: 12),
                  FilledButton.icon(
                      ...
                      children: [
                        for (final line in _lines)
                          ListTile(
                            ...
                            subtitle: Text('${line.quantity.toStringAsFixed(0)} x ${line.unitPrice.toStringAsFixed(2)}'),
                  ...
                  return Column(
                    children: [
                      for (final sale in sales)
                        ListTile(
                          title: Text(sale.customerName.isEmpty ? 'Client' : sale.customerName),
                          subtitle: Text('${sale.lines.length} ligne(s) · ${formatDateTime(sale.createdAt)}'),
  ...
  void _addLine() {
    ...
    final product = products.where((item) => item.id == productId).toList(growable: false).first;
    final quantity = double.tryParse(_quantityController.text) ?? 0;
    final price = double.tryParse(_priceController.text) ?? product.unitPrice;
    if (quantity <= 0 || price <= 0) return;
    setState(() {
      _lines.add(SaleLineItem(productId: product.id, label: product.name, quantity: quantity, unitPrice: price));
  ...
  Future<void> _saveSale() async {
    ...
    try {
      ...
      if (customerName.isNotEmpty) {
        final existing = widget.controller.customers.value.where((customer) => customer.name.toLowerCase() == customerName.toLowerCase()).toList(growable: false);
...
class _PurchasesPageState extends State<PurchasesPage> {
          ...
          children: [
            _FormCard(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  TextField(controller: _supplierName, decoration: const InputDecoration(labelText: 'Fournisseur')),
                  const SizedBox(height: 12),
                  TextField(controller: _supplierPhone, decoration: const InputDecoration(labelText: 'Téléphone fournisseur')),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                    value: _selectedProductId,
                    items: products
                  ...
                  SwitchListTile(
                    ...
                    onChanged: (value) => setState(() => _creditPurchase = value),
                    title: const Text('Achat à crédit'),
                    subtitle: const Text('Désactiver pour un paiement immédiat'),
                  ),
                  const SizedBox(height: 12),
                  TextField(
                  ...
                  return Column(
                    children: [
                      for (final purchase in purchases)
                        Card(
                          child: ListTile(
                            title: Text(purchase.supplierName.isEmpty ? 'Fournisseur' : purchase.supplierName),
                            subtitle: Text('${purchase.paymentStatus.label} · reste ${purchase.remainingAmount.toStringAsFixed(2)}'),
                            trailing: TextButton(
                              onPressed: purchase.remainingAmount <= 0 ? null : () => _payPurchase(purchase),
  ...
  void _addLine() {
    ...
    if (quantity <= 0 || price <= 0) return;
    setState(() {
      _lines.add(PurchaseLineItem(productId: product.id, label: product.name, quantity: quantity, unitPrice: price));
      _quantityController.text = '1';
      _priceController.clear();
    });
  }
  ...
  Future<void> _savePurchase() async {
    ...
    try {
      ...
      if (supplierName.isNotEmpty) {
        final existing = widget.controller.suppliers.value.where((supplier) => supplier.name.toLowerCase() == supplierName.toLowerCase()).toList(growable: false);
        if (existing.isNotEmpty) {
      ...
      final dueAt = _creditPurchase && _dueDateController.text.trim().isNotEmpty ? DateTime.tryParse('${_dueDateController.text.trim()}T00:00:00Z') : null;
  ...
  Future<void> _payPurchase(PurchaseItem purchase) async {
    final amountController = TextEditingController(text: purchase.remainingAmount.toStringAsFixed(2));
    final noteController = TextEditingController(text: 'Règlement complémentaire');
                ...
                children: [
                  for (final product in products)
                    ListTile(
                      ...
                      subtitle: Text('SKU ${product.sku.isEmpty ? '—' : product.sku}'),
                      trailing: Text('${product.stockQuantity.toStringAsFixed(0)} unités'),
                    ),
                ],
              );
            },
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(child: TextField(controller: _unit, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Prix de vente'))),
            const SizedBox(width: 12),
            Expanded(child: TextField(controller: _cost, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Coût d’achat'))),
          ],
        ),
        const SizedBox(height: 12),
        TextField(controller: _stock, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Stock initial')),
        const SizedBox(height: 12),
        if (_error != null) ...[
          Text(_error!, style: TextStyle(color: Theme.of(context).colorScheme.error)),
          const SizedBox(height: 8),
                ...
                children: [
                  for (final supplier in suppliers)
                    ListTile(
                      ...
                      subtitle: Text(supplier.phone.isEmpty ? '—' : supplier.phone),
                    ),
                ],
              );
            },
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        TextField(controller: _amount, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Montant')),
        const SizedBox(height: 12),
        if (_error != null) ...[
          Text(_error!, style: TextStyle(color: Theme.of(context).colorScheme.error)),
          const SizedBox(height: 8),
          ...
          child: Column(
            ...
            children: [
              ...
              ValueListenableBuilder<String?>(
                ...
                builder: (context, message, _) => Text(message ?? 'Aucun message de synchronisation'),
              ),
            ],
          ),
        ),
