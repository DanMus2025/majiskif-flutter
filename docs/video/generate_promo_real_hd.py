...
OUT = VIDEO_DIR / "KESE-publicite-reelle-HD-courte-rythmee.mp4"
TEMP_VIDEO = VIDEO_DIR / "KESE-publicite-reelle-HD-courte-sans-audio.mp4"
MUSIC_WAV = VIDEO_DIR / "KESE-musique-fond-rythmee.wav"
...
def scene_final(base):
    ...
    d.text((300, 470), "Gérez votre entreprise avec intelligence", font=F["h1"], fill=(255, 255, 255, 255))
    text_block(
        d,
        "Achetez votre licence KESE dès aujourd’hui et recevez gratuitement votre terminal Android POS professionnel avec KESE intégré pour imprimer vos tickets de caisse.",
        (250, 600),
        F["body_b"],
        (255, 239, 191, 255),
        1420,
...
def render_frame(index):
    ...
    if scene == 0:
        scene_intro(base, 9 + local * 1.7)
    elif scene == 1:
        scene_offline(base, local, source_t=16 + local * 2.4)
    elif scene == 2:
        scene_modules(base, source_t=30 + local * 3.0)
    elif scene == 3:
        scene_multi(base, 60 + local * 2.3, 20 + local * 3.0)
    elif scene == 4:
        scene_messages(base, source_t=20 + local * 8.0)
    elif scene == 5:
        pos_with_real_screen(base, SRC1.frame_at(45 + local * 3.0), local)
    elif scene == 6:
        scene_sync(base, local, source_t=120 + local * 3.0)
    elif scene == 7:
        scene_branding(base)
    else:
        scene_final(base)
